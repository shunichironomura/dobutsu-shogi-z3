...
def example_checkmate_solving():
    ...
    if solution:
        ...
        for move in solution.moves:
            print(f"  Move {move.move_number + 1} ({move.player}): {move.piece_type.name} "
                  f"from {move.from_} to {move.to}")
    else:
...
def example_reachability_analysis():
    ...
    if solution:
        ...
        for move in solution.moves:
            print(f"  Move {move.move_number + 1}: {move.piece_type.name} "
                  f"from {move.from_} to {move.to}")
    else:
...
def example_solver_comparison():
    ...
    for piece_id in [PieceId(0), PieceId(1), PieceId(2), PieceId(3)]:  # Sente pieces
        reachability_problem = ReachabilityProblem(
            DEFAULT_INITIAL_SETUP, piece_id, (4, 2), Player.SENTE, 5
        )
        ...
        print(f"Reachability solver - {piece_type.name}: "
              f"{'Can reach' if reachability_result else 'Cannot reach'} (4,2)")
